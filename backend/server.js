...
// // });
// app.use((req, res, next) => {
//   // Simulate an authenticated user from Clerk
//   req.auth = {
//     id: 'user_2cS5UzdbRFHlL7v959jWEpDq1rU', // Set the Clerk User ID in 'id' (or whatever you need)
//     role: 'Admin', // Set the user's role (if applicable)
...
// import { syncUser } from './controllers/userController.js'; // Remove 
...
// Temporary Authentication Bypass Middleware (For Development Only)
//  Comment out since we are removing authentication for now
// app.use((req, res, next) => {
//   // Simulate an authenticated user (replace with a real user ID from your database if needed)
//   req.auth = {
//     userId: 'user_2cS5UzdbRFHlL7v959jWEpDq1rU', // Replace this with a user ID from your Clerk dashboard for consistent testing
...
// Serve the Swagger UI at /api-docs
// app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(swaggerDocument)); //Comment out

// Use the body-parser middleware to parse raw request body
// app.use(bodyParser.raw({ type: 'application/json' })); // Comment out

// Webhook secret and headers
// const whSecret = process.env.CLERK_WEBHOOK_SECRET; // Comment out

// Handle Clerk webhooks
// app.post('/api/webhooks', async (req, res) => { // Comment out
//     if (!whSecret) {
//         console.log('CLERK_WEBHOOK_SECRET not set.');
//         return res.status(500).json({ error: 'Server configuration error' });
//       }
